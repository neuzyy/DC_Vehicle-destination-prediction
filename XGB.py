...
for out_id in test_out_id.keys():
    ...
    model = xgb.train(xgb_params,dataset_train,num_boost_round=20000,evals=watchlist)
    ...
    # b ： 二进制文件
    if n == 0:
        test.to_csv("predict_int.csv", mode='a', encoding='utf-8', index=False,header=True)
    else:
        test.to_csv("predict_int.csv", mode='a', encoding='utf-8', index=False,header=False)
