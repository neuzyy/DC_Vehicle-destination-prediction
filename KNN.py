...
for out_id in test_out_id.keys():
    ...
    # print('test',columns)
    # test ['r_key', 'out_id', 'start_time', 'start_lat', 'start_lon', 'period', 'week_code', 'start_code', 'predict']
    ...
    # b ： 二进制文件
    if n == 0:
        test.to_csv("predict.csv", mode='a', encoding='utf-8', index=False,
                    header=True)
    else:
        test.to_csv("predict.csv", mode='a', encoding='utf-8', index=False,
                    header=False)
...
print("输出结果：\n")
df = pd.read_csv("predict.csv")  # 预测结果文件
df['end_lat'] = None
df['end_lon'] = None
...
df.to_csv("predict_decode.csv", encoding='utf-8', index=False)
