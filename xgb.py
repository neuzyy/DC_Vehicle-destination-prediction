...
#              'period', 'week_code', 'start_code', 'end_code_int']
    ...
    train_x['period'] = pd.to_numeric(train_x['period'])
    train_x['week_code'] = pd.to_numeric(train_x['week_code'])
    test_x['period'] = pd.to_numeric(test_x['period'])
    test_x['week_code'] = pd.to_numeric(test_x['week_code'])
    #加载的数据存储在对象DMatrix中
    dataset_train = xgb.DMatrix(train_x,label=train_y.astype('int')) 	 	#train_x是所有特征信息，train_y是0，1代表样本的正负
    dataset_test = xgb.DMatrix(test_x)
    watchlist = [(dataset_train,'train')]
    model = xgb.train(xgb_params,dataset_train,num_boost_round=2000,evals=watchlist)
    softprob_pred = model.predict(xgb.DMatrix(test_x))
    test['end_code_int'] = softprob_pred
    #save feature score
    ...
    # b ： 二进制文件
    if n == 0:
        test.to_csv("predict_int_xgb.csv", mode='a', encoding='utf-8', index=False,header=True)
    else:
        test.to_csv("predict_int_xgb.csv", mode='a', encoding='utf-8', index=False,header=False)
...
print("输出结果：\n")
df = pd.read_csv("predict_int_xgb.csv")  # 预测结果文件
...
df.to_csv("predict_decode_xgb.csv", encoding='utf-8', index=False)
