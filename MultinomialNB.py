...
for out_id in test_out_id.keys():
    ...
    # b ： 二进制文件
    if n == 0:
        test.to_csv("predict_int_MultinomialNB.csv", mode='a', encoding='utf-8', index=False,header=True)
    else:
        test.to_csv("predict_int_MultinomialNB.csv", mode='a', encoding='utf-8', index=False,header=False)
...
print("输出结果：\n")
df = pd.read_csv("predict_int_MultinomialNB.csv")  # 预测结果文件
...
df.to_csv("predict_decode_MultinomialNB.csv", encoding='utf-8', index=False)
